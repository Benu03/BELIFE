...
class Feature_m extends CI_Model
{
    ...
    function getkodeorder()
    {
        ...
        where convert(date,date_order,103) = convert(date,getdate(),103) ");
        $kd = "";
        if($q->num_rows()>0){
    ...
    {
        $query = "select count('') as countdata from contract  with(nolock)  where user_ordeR='$username'
    ...
    {
        $query = " select count('') as countdata  from orders with(nolock)
    ...
    {
        $query = "   update voucher set is_used=1 , user_used='$username', date_used=getdate()  where kode_voucher='$kode_voucher'";   
